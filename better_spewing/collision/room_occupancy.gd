...
static func validate_context(context: Dictionary) -> String:
	...
	for field in [
		...
		"bitset", "cell_size_px", "width_cells", "height_cells", "width_px", "height_px",
	]:
	...
	if room_bytes.size() < 22 or room_bytes.slice(0, 4).get_string_from_ascii() != ROOM_MAGIC:
		return "room definition encoding magic mismatch"
	if _read_u16(room_bytes, 4) != ROOM_VERSION:
		return "room definition encoding version mismatch"
	if _read_u16(room_bytes, 6) != context.cell_size_px \
			or _read_u16(room_bytes, 8) != context.width_cells \
			or _read_u16(room_bytes, 10) != context.height_cells \
			or _read_u16(room_bytes, 12) != context.width_px \
			or _read_u16(room_bytes, 14) != context.height_px:
		return "room definition encoding dimensions mismatch"
	var identifier_length := _read_u16(room_bytes, 16)
	var rectangle_count_offset := 18 + identifier_length
	if identifier_length < 1 or rectangle_count_offset + 4 > room_bytes.size():
		return "room definition encoding identifier length mismatch"
	var rectangle_count := _read_u32(room_bytes, rectangle_count_offset)
	if rectangle_count < 0 or room_bytes.size() != rectangle_count_offset + 4 + rectangle_count * 16:
		return "room definition encoding rectangle length mismatch"
